<template>
	<SeamlessScroll class="center-bottom">
		<div v-for="item, index in actions" class="bottom-item">
			<ElIcon size="32" :color="item.color">
				<component :is="iconObj[item.icon]" />
			</ElIcon>
			<Vue3Odometer :style="{ color: item.color }" class="item-text" :value="item.value" />
		</div>
	</SeamlessScroll>
</template>
...
.center-bottom {
    ...
    align-items: center;
		background-color: var(--screen-block-bg);
		font-size: 22px;
    font-weight: 600;
		.item-text {
      margin-top: 16px;
